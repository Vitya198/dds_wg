/*module wm_data_slave(
  ...
  always @(negedge aud_bclk, negedge rst_n) begin
    if(!rst_n) begin
        ...
        if (aud_daclrc == 1'b1) begin
          if (sample_flag == 1'b1) begin
            received_data[data_index] <= aud_dacdat;
  ...
  always @(posedge aud_bclk) begin
    if (aud_daclrc == 1'b0) begin
      if (sample_flag == 1'b0) begin
        data_received   <= 1'b0;
        sample_flag     <= 1'b1;
        data_index      <= 15;
        dac_data_reg    <= received_data;
      end
    end
  end

 assign analog_data = dac_data_reg[15:8];
